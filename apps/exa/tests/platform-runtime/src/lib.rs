//! Exa app runtime conformance tests against the platform Wasm host.
...
/// Returns the Exa manifest under test.
pub fn manifest() -> AppManifest {
    let path = app_root().join("manifest.yaml");
    let yaml = fs::read_to_string(path).expect("read Exa manifest");
    AppManifest::from_yaml(&yaml).expect("Exa manifest should parse")
}

/// Returns a freshly built Exa component for runtime tests.
...
/// Returns the Exa package tested by the platform runtime harness.
...
fn build_component_bytes() -> Vec<u8> {
    ...
    let cargo_status = Command::new("cargo")
        ...
        .status()
        .expect("start Exa component cargo build");
    assert!(cargo_status.success(), "Exa component cargo build failed");

    let core_wasm = component_target_dir(&app_root, env::var_os("CARGO_TARGET_DIR"))
        .join("wasm32-unknown-unknown")
        .join("release")
        .join("fna_app_exa_component.wasm");
    ...
    assert!(
        ...
        "Exa component wasm-tools build failed"
    );

    let bytes = fs::read(&component_wasm).expect("read Exa Wasm component");
...
fn component_filename() -> String {
    let timestamp = SystemTime::now()
        ...
        .as_nanos();
    format!("fna-exa-component-{}-{timestamp}.wasm", std::process::id())
}

#[cfg(test)]
#[path = "../exa_package_tests.rs"]
mod exa_package_tests;
#[cfg(test)]
#[path = "../exa_runtime_support.rs"]
mod exa_runtime_support;
#[cfg(test)]
#[path = "../exa_tool_smoke_tests.rs"]
mod exa_tool_smoke_tests;
#[cfg(test)]
