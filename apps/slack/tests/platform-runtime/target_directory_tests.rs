//! Cargo target-directory resolution tests for the Slack component build.
...
fn component_target_dir_defaults_to_component_target() {
    let app_root = Path::new("/repo/apps/slack");

    assert_eq!(
        component_target_dir(app_root, None),
        PathBuf::from("/repo/apps/slack/component/target")
    );
}
