//! Slack app runtime conformance tests against the platform Wasm host.
...
/// Returns the Slack manifest under test.
pub fn manifest() -> AppManifest {
    let path = app_root().join("manifest.yaml");
    let yaml = fs::read_to_string(path).expect("read Slack manifest");
    AppManifest::from_yaml(&yaml).expect("Slack manifest should parse")
}

/// Returns a freshly built Slack component for runtime tests.
...
/// Returns the Slack package tested by the platform runtime harness.
...
fn build_component_bytes() -> Vec<u8> {
    ...
    let cargo_status = Command::new("cargo")
        ...
        .status()
        .expect("start Slack component cargo build");
    assert!(cargo_status.success(), "Slack component cargo build failed");

    let core_wasm = component_target_dir(&app_root, env::var_os("CARGO_TARGET_DIR"))
        .join("wasm32-unknown-unknown")
        .join("release")
        .join("fna_app_slack_component.wasm");
    ...
    assert!(
        ...
        "Slack component wasm-tools build failed"
    );

    let bytes = fs::read(&component_wasm).expect("read Slack Wasm component");
...
fn component_filename() -> String {
    let timestamp = SystemTime::now()
        ...
        .as_nanos();
    format!(
        "fna-slack-component-{}-{timestamp}.wasm",
        std::process::id()
    )
}
...
#[cfg(test)]
#[path = "../slack_package_tests.rs"]
mod slack_package_tests;
#[cfg(test)]
#[path = "../slack_runtime_support.rs"]
mod slack_runtime_support;
#[cfg(test)]
#[path = "../slack_tool_smoke_tests.rs"]
mod slack_tool_smoke_tests;
#[cfg(test)]
#[path = "../slack_url_verification_tests.rs"]
mod slack_url_verification_tests;
#[cfg(test)]
