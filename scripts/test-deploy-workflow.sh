...
require_line "$deploy_workflow" '              --apps-root apps > /tmp/deploy-apps'
require_line "$deploy_workflow" 'selection_mode=force_all'
require_line "$deploy_workflow" 'selection_mode=explicit'
require_line "$deploy_workflow" 'selection_mode=automatic'
require_line "$deploy_workflow" 'echo "mode=$selection_mode" >> "$GITHUB_OUTPUT"'
require_line "$deploy_workflow" 'if [ "$deployment_mode" = "explicit" ]; then'
require_line "$deploy_workflow" 'planner_mode_args=(--force)'
...
reject_line "$deploy_workflow" 'steps.selection.outputs.forced'
